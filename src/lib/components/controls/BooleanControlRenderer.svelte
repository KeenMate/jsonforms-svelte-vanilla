<script lang="ts">
	...
	const styles = $derived.by(() => {
		...
		if (parentUserStyles) {
			...
			// console.log("Result of styles merge with parentUserStyles", resultStyles, parentUserStyles)
		} else {
			merge(resultStyles, defaultStyles)
			// console.log("Result of styles merge with defaultStyles", resultStyles, defaultStyles)
		}
		if (element?.options?.styles) {
			merge(resultStyles, element.options.styles)
			// console.log("Result of styles merge with element.options.styles", resultStyles, element.options.styles)
		}
		// console.log("Result of styles merge with ", resultStyles)
		return resultStyles
	})
	...
	$effect(() => {
	 ...
	 // $inspect("What is config?", config)
	 // $inspect("What is uischema?", control.uischema)
	 // $inspect("What is styles?", styles.control)
	...
	function onChange(event: Event) {
		console.log("Boolean on change", event)
		input.handleChange(control.path, event.target.checked || undefined)
	}
</script>
